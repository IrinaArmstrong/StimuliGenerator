...
class CurveFit:
  ...
  def leftTangent(self, data, tolerance):
    """
    Compute left tangent to curve.
    :param data: input points;
    :param tolerance: error;
    :return:
    """
    i = 1
    while True:
      pi = data[i]
      t = pi - data[0]
      distSq = t.__matmul__(t) # Dot product: t*t
  ...
  def generateBezier(self, data, u, tHat1, tHat2, tolerance_sq):
    ...
    :param tHat1: tangent 1;
    :param tHat2: tangent 2;
    :param tolerance_sq: error in fitting;
    :return:
    """
    est_tHat1 = tHat1
    est_tHat2 = tHat2
    if not est_tHat1: # If no tangent given
      est_tHat1 = self.leftTangent(data, tolerance_sq)
    if not est_tHat2: # If no tangent given
  ...
  def chordLengthParameterize(self, points):
    """
    Compute distances along the polygon, scaled to the [0,1] domain.
    :param points: input Points list;
    :return: scaled distances vector.
    """
    u = [0.0]
    v = 0
  ...
  def _fitCurve(self, points, tangent1, tangent2, error, cornerTolerance, maxSegments):
    ...
    maxIterations = 3
    isCorner = False
    u = self.chordLengthParameterize(points) # Get distances vector
    if u[-1] == 0.0: return [] # If all points are equal then no curve
